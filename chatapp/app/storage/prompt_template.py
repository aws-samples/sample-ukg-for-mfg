...
class PromptTemplateStorageService:
    ...
    async def create_template_with_id(
        ...
        """Create a template using a caller-supplied template_id (for bulk upload / seeding)."""
        try:
            now = datetime.utcnow().isoformat()
            template = PromptTemplate(
                template_id=template_id,
            ...
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self._put_item_sync, template)
            logger.info("Created prompt template (with id)", extra={"template_id": template_id, "title": title})
            return template
        except Exception as e:
            logger.error("Failed to create template with id", extra={"template_id": template_id, "error": str(e)})
            return None
