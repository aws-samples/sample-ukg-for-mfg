...
async def list_templates() -> JSONResponse:
    """List all prompt templates sorted by sort_order. Built-in defaults always appear first."""
    ...
    # Prepend built-in defaults, skipping any that already exist in DDB by template_id
    existing_ids = {t.get("template_id") for t in result}
    prepend = [d for d in DEFAULT_TEMPLATES if d["template_id"] not in existing_ids]
...
async def create_template(
    ...
    if template:
        ...
        if is_ajax:
            return JSONResponse(content={"success": True, "template": template.to_dict()})
    else:
...
async def reorder_templates(request: Request) -> JSONResponse:
    ...
    if not order:
        return JSONResponse(content={"success": False, "error": "No order provided"}, status_code=400)

    storage = PromptTemplateStorageService()
    for idx, template_id in enumerate(order):
        await storage.update_sort_order(template_id, idx)
...
async def bulk_delete_templates(request: Request) -> JSONResponse:
    ...
    if not isinstance(ids, list) or not ids:
        return JSONResponse(content={"success": False, "error": "Expected non-empty 'template_ids' array"}, status_code=400)
    ...
    logger.info("Bulk deleted templates", extra={"requested": len(ids), "deleted": deleted})
...
async def bulk_upload_templates(request: Request) -> JSONResponse:
    ...
    if not isinstance(templates_data, list) or len(templates_data) == 0:
        return JSONResponse(
            content={"success": False, "error": "Expected a non-empty 'templates' array"},
            status_code=400,
        )
    ...
    return JSONResponse(content={"success": True, "created": len(created), "templates": created})
