...
LAMBDA_FUNCTION_NAME="${LAMBDA_FUNCTION_NAME:-${APP_NAME}-lambda}"
...
deploy_ecs() {
    ...
    # ECS Express uses standard UpdateService for deployment config (not UpdateExpressGatewayService)
    echo -e "${CYAN}Resetting deployment configuration (bake=0, canary=100%)...${NC}"
    aws ecs update-service \
        --cluster default \
        --service "$ECS_SERVICE_NAME" \
        ...
        echo -e "${YELLOW}⚠ Could not update deployment configuration (non-fatal)${NC}"
