    ...
    const triggerEcsBuild = new cr.AwsCustomResource(this, 'TriggerEcsBuild', {
      onCreate: {
        ...
        parameters: {
          ...
          // environmentVariablesOverride is part of the API call parameters — CFN diffs
          // these, so changing DEPLOY_TIMESTAMP on every synth forces onUpdate to fire.
          environmentVariablesOverride: [
            { name: 'DEPLOY_TIMESTAMP', value: buildTimestamp, type: 'PLAINTEXT' },
          ],
        },
        physicalResourceId: cr.PhysicalResourceId.fromResponse('build.id'),
        // Only store build.id — full CodeBuild response exceeds CFN's 4KB limit
...
`),
      ...
      primaryContainer: {
        ...
        secrets: [
          ...
          },
          {
            ...
            valueFrom: `${secretArn}:registry_table_name::`,
          },
          {
            name: 'DISCOVERY_RUNTIME_ARN',
          ...
          },
          {
            ...
            valueFrom: `${secretArn}:discovery_eval_config_id::`,
          },
          {
            name: 'WORKFLOW_EXECUTOR_ARN',
            valueFrom: `${secretArn}:workflow_executor_arn::`,
          },
          {
            name: 'WORKFLOWS_TABLE_NAME',
            valueFrom: `${secretArn}:workflows_table_name::`,
          },
          {
            name: 'WORKFLOW_SCHEDULER_GROUP',
            valueFrom: `${secretArn}:workflow_scheduler_group::`,
          },
          {
            name: 'WORKFLOW_SCHEDULER_ROLE_ARN',
            valueFrom: `${secretArn}:workflow_scheduler_role_arn::`,
          },
          {
            name: 'WORKFLOW_RESULTS_TABLE_NAME',
            valueFrom: `${secretArn}:workflow_results_table_name::`,
          },
        ],
    ...
    // ========================================================================
    // ECS Express deployment config (bakeTimeInMinutes, canaryPercent) is set
    // via the standard ecs:UpdateService API after each deploy. This is handled
    // by deploy-all.sh and chatapp/deploy.sh automatically.
    // CfnExpressGatewayService does not expose these settings as CFN properties.
  }
    ...
    const triggerLambdaBuild = new cr.AwsCustomResource(this, 'TriggerLambdaBuild', {
      onCreate: {
        ...
        parameters: {
          ...
          sourceLocationOverride: `${this.sourceBucket.bucketName}/chatapp-source/`,
          environmentVariablesOverride: [
            { name: 'DEPLOY_TIMESTAMP', value: lambdaBuildTimestamp, type: 'PLAINTEXT' },
          ],
        },
        physicalResourceId: cr.PhysicalResourceId.fromResponse('build.id'),
        // Only store build.id — full CodeBuild response exceeds CFN's 4KB limit
...
`),
    ...
    const secretFields: { [key: string]: string } = {
      ...
      'WORKFLOW_RESULTS_TABLE_NAME': 'workflow_results_table_name',
    };
    
    // Add each secret as an environment variable using dynamic secret reference
    // Format: {{resolve:secretsmanager:secretId:SecretString:jsonKey}}
    // This resolves at CloudFormation deploy time, not CDK synthesis time
    for (const [envVar, secretField] of Object.entries(secretFields)) {
      this.lambdaFunction.addEnvironment(
        envVar,
        `{{resolve:secretsmanager:${secretArn}:SecretString:${secretField}}}`
      );
    }
...
exports.handler = async (event) => {
  ...
  return request;
    ...
    // Create a region-unique OAC to avoid collisions in multi-region deployments
    // (CloudFront OACs are global resources with names derived from construct paths)
    const functionUrlOac = new cloudfront.FunctionUrlOriginAccessControl(this, 'FunctionUrlOAC', {
      originAccessControlName: `${config.appName}-furl-oac-${this.region}`,
    });
