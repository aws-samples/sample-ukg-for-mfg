...
export class FoundationStack extends cdk.Stack {
  ...
  /** System Registry table for V2 Scalable Knowledge Layer */
  public readonly systemRegistryTable: dynamodb.Table;

  /** Discovery History table for tracking discovery sessions */
  public readonly discoveryHistoryTable: dynamodb.Table;

  /** Workflows table for saved repeatable workflows and results */
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // System Registry table for V2 Scalable Knowledge Layer
    // Single-table design with two GSIs for concept and equivalence lookups
    this.systemRegistryTable = new dynamodb.Table(this, 'SystemRegistryTable', {
      tableName: `${config.appName}-system-registry`,
      partitionKey: {
        name: 'PK',
        type: dynamodb.AttributeType.STRING,
      },
    ...
    // GSI1: Concept-to-system lookups (CONCEPT#{concept_id} → systems/fields)
    this.systemRegistryTable.addGlobalSecondaryIndex({
      indexName: 'GSI1',
      partitionKey: {
        name: 'GSI1PK',
        type: dynamodb.AttributeType.STRING,
    ...
    // GSI2: Cross-system field equivalence lookups (EQUIV#{concept_id} → source fields)
    this.systemRegistryTable.addGlobalSecondaryIndex({
      indexName: 'GSI2',
      partitionKey: {
        name: 'GSI2PK',
    ...
    // Discovery History table for tracking discovery sessions
    this.discoveryHistoryTable = new dynamodb.Table(this, 'DiscoveryHistoryTable', {
      tableName: config.discoveryHistoryTableName,
      partitionKey: {
        name: 'discovery_id',
        type: dynamodb.AttributeType.STRING,
      },
    ...
    // GSI for querying by system_id (find all discovery sessions for a system)
    this.discoveryHistoryTable.addGlobalSecondaryIndex({
    ...
    // Workflows table — workflow definitions keyed by user (PK=user_email, SK=workflow_id)
    this.workflowsTable = new dynamodb.Table(this, 'SavedWorkflowsTable', {
      tableName: config.workflowsTableName,
      partitionKey: {
        name: 'user_email',
        type: dynamodb.AttributeType.STRING,
      },
    ...
    // Workflow results table — execution results (PK=workflow_id, SK=timestamp)
    const workflowResultsTable = new dynamodb.Table(this, 'WorkflowResultsTable', {
      tableName: config.workflowResultsTableName,
      partitionKey: {
        name: 'workflow_id',
        type: dynamodb.AttributeType.STRING,
      },
    ...
    this.taskRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          ...
          `${this.systemRegistryTable.tableArn}/index/*`,
          this.discoveryHistoryTable.tableArn,
          `${this.discoveryHistoryTable.tableArn}/index/*`,
          this.workflowsTable.tableArn,
          `${this.workflowsTable.tableArn}/index/*`,
          workflowResultsTable.tableArn,
          `${workflowResultsTable.tableArn}/index/*`,
        ],
      })
    );
    ...
    // EventBridge Scheduler — manage workflow schedules
    this.taskRole.addToPolicy(
      new iam.PolicyStatement({
        sid: 'SchedulerAccess',
        effect: iam.Effect.ALLOW,
        ...
        resources: [
          `arn:aws:scheduler:${this.region}:${this.account}:schedule/${config.appName}-workflows/*`,
        ],
      })
    );
    ...
    this.taskRole.addToPolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          `arn:aws:iam::${this.account}:role/${config.appName}-workflow-scheduler-role-${this.region}`,
        ],
      })
    );

    // Lambda invocation — for triggering workflow executor from the chatapp
    this.taskRole.addToPolicy(
      new iam.PolicyStatement({
        sid: 'LambdaInvokeWorkflowExecutor',
        effect: iam.Effect.ALLOW,
        actions: ['lambda:InvokeFunction'],
        resources: [
          `arn:aws:lambda:${this.region}:${this.account}:function:${config.appName}-workflow-executor`,
        ],
      })
    );
    ...
    // Read evaluation results from CloudWatch Logs
    this.taskRole.addToPolicy(
      new iam.PolicyStatement({
        sid: 'EvaluationLogsRead',
        effect: iam.Effect.ALLOW,
        actions: [
    ...
    this.secret = new secretsmanager.Secret(this, 'AppConfigSecret', {
      ...
      secretObjectValue: {
        ...
        workflows_table_name: cdk.SecretValue.unsafePlainText(this.workflowsTable.tableName),
        workflow_results_table_name: cdk.SecretValue.unsafePlainText(workflowResultsTable.tableName),
        
        // Downstream-managed keys: seeded with empty placeholders so that
        // {{resolve:secretsmanager:...}} references don't fail. Real values
        // are written by Bedrock, Agent, and Gateway stacks via merge custom
        ...
        workflow_scheduler_role_arn: cdk.SecretValue.unsafePlainText(''),
      },
    ...
    new cdk.CfnOutput(this, 'SystemRegistryTableName', {
      value: this.systemRegistryTable.tableName,
      description: 'System Registry DynamoDB table name',
      exportName: `${config.appName}-SystemRegistryTableName`,
    });

    new cdk.CfnOutput(this, 'SystemRegistryTableArn', {
      value: this.systemRegistryTable.tableArn,
      description: 'System Registry DynamoDB table ARN',
      exportName: `${config.appName}-SystemRegistryTableArn`,
    });
      ...
      [
        ...
        },
        {
          ...
          appliesTo: [
            ...
            'Resource::<SystemRegistryTableC2FC1C78.Arn>/index/*',
            'Resource::<WorkflowResultsTable541719BD.Arn>/index/*',
          ],
        },
        {
          id: 'AwsSolutions-IAM5',
          reason: 'EventBridge Scheduler schedules are created at runtime with dynamic names. Scoped to app-specific schedule group.',
          appliesTo: [`Resource::arn:aws:scheduler:${this.region}:${this.account}:schedule/${config.appName}-workflows/*`],
        },
        {
          id: 'AwsSolutions-IAM5',
